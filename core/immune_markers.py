...
def _reference_values_for_gene(sample: str, gene: str) -> tuple[list[float], float | None, str]:
    ...
    return reference_values, patient_value, source_label


def _histogram_svg_from_reference(sample_value: float, reference_values: list[float], legend_left: str = "референс", legend_right: str = "образец") -> str:
    ...
    for value in reference_values:
        idx = int((value - x_min) / max(x_max - x_min, 1e-6) * bins)
        idx = max(0, min(bins - 1, idx if value < x_max else bins - 1))
        counts[idx] += 1

    max_count = max(max(counts, default=0), 1)
    sample_bin = int((sample_value - x_min) / max(x_max - x_min, 1e-6) * bins)
    sample_bin = max(0, min(bins - 1, sample_bin if sample_value < x_max else bins - 1))

    # Оранжевый столбец теперь зависит от фактического значения пациента,
    # а не копирует целиком высоту синего бина.
    sample_count_height = ((sample_value - x_min) / max(x_max - x_min, 1e-6)) * max_count
    sample_count_height = max(0.0, sample_count_height)
    frame_count_height = max(counts[sample_bin], sample_count_height)
    max_count_plot = max(max_count, frame_count_height, 1)

    bars: list[str] = []
    selected_outline = ""
    bin_w = plot_w / bins
    sample_bar_w = max(3.0, bin_w * 0.52)
    sample_bar_h_px = max(0.0, (sample_count_height / max_count_plot) * plot_h)

    for idx, count in enumerate(counts):
        bar_h_px = (count / max_count_plot) * plot_h
        x = left + idx * bin_w + 1
        y = top + plot_h - bar_h_px
        w = max(3.0, bin_w - 2)
        h = max(0.0, bar_h_px)
        bars.append(f'<rect x="{x:.1f}" y="{y:.1f}" width="{w:.1f}" height="{h:.1f}" rx="1.5" fill="#8FA8EE" />')

        if idx == sample_bin:
            sample_x = x + (w - sample_bar_w) / 2
            sample_y = top + plot_h - sample_bar_h_px
            if sample_bar_h_px > 0:
                bars.append(f'<rect x="{sample_x:.1f}" y="{sample_y:.1f}" width="{sample_bar_w:.1f}" height="{sample_bar_h_px:.1f}" rx="1.5" fill="#F09984" />')
            frame_h_px = (frame_count_height / max_count_plot) * plot_h
            frame_y = top + plot_h - frame_h_px
            selected_outline = f'<rect x="{x:.1f}" y="{frame_y:.1f}" width="{w:.1f}" height="{frame_h_px:.1f}" rx="1.5" fill="none" stroke="#111827" stroke-width="2" />'
...
def _build_card(card_cfg: dict[str, Any], reference: dict[str, Any], expression_values: dict[str, float], sample: str) -> dict[str, Any]:
    ...
    reference_values, cohort_patient_value, source_label = _reference_values_for_gene(sample, gene)
    ...
    if reference_values:
        ...
        svg_uri = _svg_to_data_uri(_histogram_svg_from_reference(sample_value, reference_values, legend_left=source_label, legend_right="образец"))
