...
# Это функция для чтения значения TMB из файла с результатами анализа.
...
# Это функция для классификации значения TMB по заранее заданным диапазонам и подготовки текста для отображения в интерфейсе.
...
# Это функция для перевода значения TMB в позицию маркера на визуальной шкале.
# Используется в следующих блоках: sections/genetic_profile_block.py.
# На вход: value, max_value.
# На выход: float.
...
# Это функция для блока «severity rank». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: core/genetic_profile.py.
# На вход: consequence.
# На выход: int.
...
# Это функция для блока «protein hit». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: core/genetic_profile.py.
# На вход: gene, protein_change, consequence.
# На выход: bool.
...
# Это функция для загрузки и отбора ключевых соматических вариантов, которые затем показываются в генетическом профиле.
# Используется в следующих блоках: core/genetic_profile.py.
# На вход: path_str, mtime_ns.
# На выход: таблицу pandas.DataFrame.
...
@lru_cache(maxsize=128)
# Это функция для загрузки HLA-аллелей и приведения их к формату, удобному для вывода в карточке пациента.
# Используется в следующих блоках: core/genetic_profile.py.
# На вход: path_str, mtime_ns.
# На выход: кортеж[кортеж[str, str, str, str], ...].
...
# Это функция для загрузки данных, связанных с блоком «загрузки редких событий текста». 
