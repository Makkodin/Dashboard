...
def _legend_html(composition: list[dict]) -> str:
    items: list[str] = []
    for item in composition:
        items.append(
            "".join(
                [
                    '<div class="immune-status-legend-item">',
                    f'<span class="immune-status-legend-dot" style="background:{esc(item["color"])};"></span>',
                    f'<span class="immune-status-legend-text">{esc(item["label"])} </span>',
                    '</div>',
                ]
            )
        )
...
def render_immune_status_block():
    ...
    html = "".join(
        [
            ...
            f'<div class="immune-status-legend">{_legend_html(composition)}</div>',
            '</div>',
            '<div class="immune-status-deconv-layout">',
            '<div class="immune-status-stack-col">',
            '<div class="immune-status-stack-axis">100%</div>',
            '<div class="immune-status-stack-axis immune-status-stack-axis--mid">50%</div>',
            '<div class="immune-status-stack-axis immune-status-stack-axis--low">25%</div>',
