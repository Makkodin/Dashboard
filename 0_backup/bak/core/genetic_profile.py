...
TMB_THRESHOLD_TEXT = "Пороги: <10 — в пределах нормы; 10–12 — выше нормы; >12 — высокая нагрузка"
...
def classify_tmb(value: float | None) -> tuple[str, str, str]:
    if value is None:
        return "Нет данных", "#6B7280", "Не удалось определить TMB"

    if value < 10:
        return "В пределах нормы", "#16A34A", TMB_THRESHOLD_TEXT
    if 10 <= value <= 12:
        return "Выше нормы", "#F59E0B", TMB_THRESHOLD_TEXT
    return "Высокая нагрузка", "#DC2626", TMB_THRESHOLD_TEXT
