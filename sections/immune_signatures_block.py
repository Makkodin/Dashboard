...
# Это функция для блока «mix channel». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
...
# Это функция для блока «score to fill». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: sections/immune_signatures_block.py.
# На вход: value.
# На выход: str.
...
# Это функция для блока «summary html». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: sections/immune_signatures_block.py.
# На вход: summary_text, summary_icon, summary_tone.
# На выход: str.
...
# Это функция для блока «item тултипа текста». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: sections/immune_signatures_block.py.
# На вход: item.
# На выход: str.
...
# Это функция для блока «items html». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: sections/immune_signatures_block.py.
# На вход: items.
# На выход: str.
...
# Используется в следующих блоках: sections/immune_markers_block.py, sections/immune_signatures_block.py.
# На вход: left_group, right_group, left_narrow, row_class.
# На выход: str.
...
# Это функция для отрисовки блока «иммунного профиля иммунных сигнатур block» в интерфейсе дашборда.
...
def render_immune_signatures_block():
    ...
    if not groups:
        _render_html(''.join([
            ...
            '<div class="immune-sign-card">',
            f'<div class="immune-sign-card-title">{esc(data.get("missing_message") or "Иммунные сигнатуры отсутствуют")}</div>',
            '<div class="immune-sign-card-desc">В таблице scores2025_2026.tsv нет строки для выбранного образца.</div>',
            '</div>',
            '</div>',
        ]))
        return
