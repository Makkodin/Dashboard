...
CELL_TYPE_TOOLTIPS = {
    "B клетки": "B-клетки участвуют в гуморальном иммунном ответе, распознавании антигенов и продукции антител.",
    "CD8 Т-клетки": "CD8 Т-клетки — основные цитотоксические лимфоциты, способные напрямую уничтожать опухолевые клетки.",
    "Фибробласты": "Фибробласты формируют и ремоделируют внеклеточный матрикс, влияя на фиброз и структуру опухолевой стромы.",
    "NK клетки": "NK-клетки обеспечивают быстрый врождённый противоопухолевый ответ и распознают клетки со сниженной презентацией антигенов.",
    "Другие": "Категория «Другие» объединяет клетки, которые не входят в основные явно показанные популяции деконволюции.",
    "CD4 Т-клетки": "CD4 Т-клетки координируют иммунный ответ, поддерживают активацию цитотоксических лимфоцитов и B-клеток.",
    "Эндотелий": "Эндотелиальные клетки выстилают сосуды и отражают ангиогенез и сосудистый компонент микроокружения опухоли.",
    "Моноцит. клетки": "Моноцитарные клетки относятся к миелоидному звену иммунитета и могут участвовать как во воспалении, так и в иммуносупрессии.",
    "T-рег. лимфоциты": "Регуляторные T-клетки подавляют иммунный ответ и могут ослаблять противоопухолевую активность лимфоцитов.",
}
...
# Это функция для блока «metric label html». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: sections/immune_status_block.py.
# На вход: text, tooltip.
# На выход: str.
...
# Это функция для блока «purity tone». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: sections/immune_status_block.py.
# На вход: purity_percent.
# На выход: кортеж[str, str].
...
# Это функция для блока «stack bar html». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: sections/immune_status_block.py.
# На вход: composition.
# На выход: str.
...
# Это функция для блока «composition rows html». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: sections/immune_status_block.py.
# На вход: composition.
# На выход: str.
def _composition_rows_html(composition: list[dict]) -> str:
    ...
    for item in composition:
        item_tooltip = CELL_TYPE_TOOLTIPS.get(
            item["label"],
            "Эта популяция отражает один из компонентов клеточного состава опухолевого микроокружения."
        )
        rows.append(''.join([
...
# Это функция для блока «placeholder class». Она нужна для того, чтобы вынести отдельный шаг логики в самостоятельную и читаемую часть кода.
# Используется в следующих блоках: sections/immune_status_block.py.
# На вход: flag.
# На выход: str.
def _placeholder_class(flag: bool) -> str:
    return " data-placeholder-frame" if flag else ""


# Это функция для отрисовки блока «иммунного профиля иммунного статуса block» в интерфейсе дашборда.
...
def render_immune_status_block() -> None:
    ...
    html = ''.join([
        ...
        '</div></div></div></div>',
        f'<div class="immune-status-card immune-status-card--subtype{_placeholder_class(bool(quality.get("subtype_is_placeholder")))}">',
        ...
        '</div></div>',
        f'<div class="immune-status-card immune-status-card--deconv{_placeholder_class(bool(quality.get("composition_is_placeholder")))}">',
