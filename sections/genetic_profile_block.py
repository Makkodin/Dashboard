...
HLA_HETERO_SECOND_COLOR = "#6CBF84"
HLA_HOMO_COLOR = "#6CBF84"

TMB_TOOLTIP_TEXT = "Пороговые значения TMB: менее 10 — в пределах нормы; от 10 до 12 — выше нормы; более 12 — высокая мутационная нагрузка."
...
def render_genetic_profile_block():
    ...
    with col_left:
        render_tmb_block(sample)

    with col_mid:
        render_somatic_block(sample)

    with col_right:
        render_hla_block(sample)

    render_rare_events_block(sample)


def render_tmb_block(sample: str):
    ...
    html = "".join(
        [
            '<div class="genetic-card genetic-card-top-row genetic-card-tmb">',
            ...
            <div class="genetic-tooltip">
                ℹ Пороговые значения TMB
                <span class="genetic-tooltip-text">{esc(TMB_TOOLTIP_TEXT)}</span>
            </div>
            ''',
            "</div>",
            "</div>",
...
def render_somatic_block(sample: str):
    df = load_key_somatic_variants(vep_ann_path(sample))

    if df.empty:
        html = "".join(
            [
                '<div class="genetic-card genetic-card-top-row genetic-card-somatic">',
                '<div class="genetic-card-head">',
                '<div class="genetic-subblock-title-row genetic-subblock-title-row--compact">',
    ...
    html = "".join(
        [
            ...
            "<tr>",
            "<th>Ген</th>",
            "<th>Белковое изменение</th>",
            "<th>Нуклеотидное изменение</th>",
            "</tr>",
            "</thead>",
            "<tbody>",
            rows_html,
...
def render_hla_block(sample: str):
    hla_rows = load_hla_class_i(norma_alleles_path(sample))

    if not hla_rows:
        html = "".join(
            [
                '<div class="genetic-card genetic-card-top-row genetic-card-hla">',
                '<div class="genetic-card-head">',
                '<div class="genetic-subblock-title-row genetic-subblock-title-row--compact">',
    ...
    for locus in loci:
        item = by_locus.get(
            locus,
            {"allele_1": "—", "allele_2": "—", "status": "Гетерозигота"},
        )
...
def render_rare_events_block(sample: str):
    ...
    with left:
        _render_html(
            "".join(
                [
                    '<div class="genetic-subblock-title-row">',
